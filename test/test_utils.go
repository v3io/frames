...
func initializeNullColumnsBitmask(length int) []*pb.RowBitmask{
	nullValues := make([]*pb.RowBitmask, length)
	for i := 0 ; i < length; i++{
		nullValues[i] = &pb.RowBitmask{NullColumns: make(map[string]bool)}
	}
	return nullValues
}